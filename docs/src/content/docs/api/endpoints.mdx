...
| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/api/lyrics` | Look a track's lyrics up through the enabled providers |

`/api/lyrics` requires `artist` and `title` query parameters and accepts
optional `album` and `duration` (seconds) to narrow the match. Providers are
asked in priority order until one answers. The response carries `provider`,
`instrumental`, `plain`, and `synced`; `synced` is an LRC-style string and is
empty when only unsynchronised lyrics exist. A track no provider knows returns
`404`.
...
| `GET` | `/api/settings/lyrics` | Read lyrics provider settings metadata |
| `POST` | `/api/settings/lyrics/:key/test` | Test a lyrics provider |
